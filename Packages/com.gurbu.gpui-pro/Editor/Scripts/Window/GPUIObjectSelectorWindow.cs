...
namespace GPUInstancerPro
{
    public class GPUIObjectSelectorWindow : GPUIEditorWindow
    {
        ...
        public static GPUIObjectSelectorWindow ShowWindow(string filter, bool showInstanceCounts, bool allowModelPrefabs, bool allowMultiSelect, bool allowSkinnedMeshRenderer, UnityAction<List<UnityEngine.Object>> prefabSelectionAction)
        {
        ...
        {
            EditorUtility.DisplayProgressBar("Loading Prefabs", "Loading Prefabs...", 0f);
            ...
            {
                if (i % 11 == 10)
                    EditorUtility.DisplayProgressBar("Loading Prefabs", "Loading Prefabs...", (float)i / guidCount);
        ...
        public bool TryGetPrefab(string path, out GameObject prefab)
        {
            ...
            if (isPrefab)
            {
                if (fi.Length > (1024 * 100)) // Skip container prefabs with sizes over 100 KB.
                    return false;

                string contents = File.ReadAllText(path);

                if (contents.Contains("MeshRenderer"))
                {
                    prefab = (GameObject)AssetDatabase.LoadMainAssetAtPath(path);
                    return prefab != null;
                }

                if (_allowSkinnedMeshRenderer && contents.Contains("SkinnedMeshRenderer"))
                {
                    prefab = (GameObject)AssetDatabase.LoadMainAssetAtPath(path);
                    return prefab != null;
                }
