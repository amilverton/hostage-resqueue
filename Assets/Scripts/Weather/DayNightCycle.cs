...
public class DayNightCycle : MonoBehaviour {
    ...
    public void UpdateFX() {
        ...
        // Set ambient light color from the ambient gradient
        Color ambientLightColor = ambientColor.Evaluate(currentTimeOfDay);
        RenderSettings.ambientLight = ambientLightColor;

        // Configure fog color and density for the current time of day
        Color fogColor = nightDayFogColor.Evaluate(currentTimeOfDay);
        RenderSettings.fogColor = fogColor; // Fog color depends on the time of day
        ...
        // Update Height Fog Global if assigned
        if (heightFogGlobal != null) {
            // Set directional color to match the sun color
            heightFogGlobal.directionalColor = currentColor;
            
            // Set fog color start to match the current fog color from the gradient
            heightFogGlobal.fogColorStart = fogColor;
        }
    }
